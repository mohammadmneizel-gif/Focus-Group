...
const translations = {
  ar: {
    ...
    submitBtn: "إرسال التقييم", backToScriptBtn: "العودة للنص", thankYouTitle: "شكراً لك", thankYouSubtitle: "تم استلام تقييمك بنجاح",
  ...
  en: {
    ...
    submitBtn: "Submit Feedback", backToScriptBtn: "Back to Script", thankYouTitle: "Thank You", thankYouSubtitle: "Your feedback has been received",
...
const pageNotes = JSON.parse(sessionStorage.getItem("ofg_pageNotes") || "{}");
let compiledNotes = "";
for (const [page, note] of Object.entries(pageNotes)) {
  if (note.trim()) compiledNotes += `Page ${parseInt(page) + 1}:\n${note}\n\n`;
}
if (compiledNotes) {
  document.getElementById("additionalComments").value = "ملاحظات الصفحات / Page Notes:\n\n" + compiledNotes;
}
...
form.addEventListener("submit", (e) => {
  ...
  const record = {
    ...
    scriptId: scriptId, scriptTitle: sessionStorage.getItem("ofg_scriptTitle"),
    fullName: fullName,
    phone: phone,
